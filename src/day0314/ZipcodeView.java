...
public class ZipcodeView extends JFrame implements ActionListener {
	...
	/**
	 * DefaultTableModel에 조회된 우편번호를 설정하는 일
	 */
...
//			List<ZipcodeVO> listZipcode=zDAO.selectStatement(dong);
			...
			//조회된 데이터를 DefaultTableModel에 설정하기 위한 배열 선언
			Object[] rowData=null;
			StringBuilder sbAddr=new StringBuilder();
			
			for(ZipcodeVO zVO : listZipcode){
				rowData=new Object[2];//우편번호, 주소(시도,구군,동, 번지로 구성)
				//우편번호 설정
				...
				//설정된 우편번호와 주소를 가진 배열을 DefaultTableModel 설정
