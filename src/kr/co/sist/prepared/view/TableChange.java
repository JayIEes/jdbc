...
public class TableChange extends JFrame implements ActionListener {
	...
	public void searchEmp(String depart) {//객체가 생성되지 않은 상태에서 String이 들어올 수도 있기 때문에
		switch ( JOptionPane.showConfirmDialog(this, depart+"의 모든 사원을 검색하시겠습니까?")){
		case JOptionPane.OK_OPTION:
			String tableName="EMP";
//			if(depart.equals("지사")) {//cp_emp5 //depart가 null이면 NullPointerException 발생
			if("지사".equals(depart)) { //얘를 더 권장 // NullPointerException을 발생시키지 않음
				tableName="CP_EMP5";
			}//end if
